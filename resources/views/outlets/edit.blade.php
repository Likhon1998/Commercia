<x-layouts.sidebar>
    <x-slot name="header">
        <h2 class="text-lg font-semibold text-gray-900 border-b pb-2">✏️ Edit Outlet</h2>
    </x-slot>

    <div class="max-w-3xl mx-auto mt-6 bg-white p-4 rounded-md shadow text-xs">
        <form action="{{ route('outlets.update', $outlet->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <section>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                        ...
                        <input id="name" name="name" type="text" required value="{{ old('name', $outlet->name) }}"
                    ...
                    <div>
                        ...
                        <input id="phone" name="phone" type="text" value="{{ old('phone', $outlet->phone) }}"
                               class="block w-full rounded border border-gray-300 px-1.5 py-0.5 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-xs">
                        @error('phone')
                            <p class="text-[10px] text-red-600 mt-0.5">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input id="email" name="email" type="email" value="{{ old('email', $outlet->email) }}"
                               class="block w-full rounded border border-gray-300 px-1.5 py-0.5 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-xs">
                        @error('email')
                            <p class="text-[10px] text-red-600 mt-0.5">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="address" name="address" rows="2"
                                  class="block w-full rounded border border-gray-300 px-1.5 py-0.5 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 resize-none text-xs">{{ old('address', $outlet->address) }}</textarea>
                        @error('address')
                            <p class="text-[10px] text-red-600 mt-0.5">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex space-x-4 items-center">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="status" value="active"
                               {{ old('status', $outlet->status) === 'active' ? 'checked' : '' }}
                    ...
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="default" value="1"
                               {{ old('default', $outlet->default) ? 'checked' : '' }}
                               class="form-checkbox h-3.5 w-3.5 text-blue-600 border-gray-300 focus:ring-blue-500">
