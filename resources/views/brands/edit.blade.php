<x-layouts.sidebar>
    <x-slot name="header">
        <h2 class="text-lg font-semibold text-indigo-700">Edit Brand</h2>
    </x-slot>
    ...
    <div class="flex justify-center mt-8 px-4">
        <div class="w-full max-w-xl bg-gradient-to-br from-indigo-50 via-white to-indigo-50 rounded-lg shadow-lg p-6">
            ...
            <form action="{{ route('brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                @if ($brand->image)
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Current Brand Image</label>
                        <img src="{{ asset('storage/brands/' . $brand->image) }}" alt="Brand Image" class="h-20 rounded-md object-contain">
                    </div>
                @endif

                {{-- Brand Image Upload --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Change Brand Image</label>
                    <input
                        type="file"
                        name="image"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-400"
