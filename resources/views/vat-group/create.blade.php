<x-layouts.sidebar>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">➕ Add New VAT Group</h2>
    ...
    <div class="max-w-lg mx-auto mt-6 p-6 bg-white shadow-md rounded-md">
        <form action="{{ route('vat-group.store') }}" method="POST" novalidate>
            @csrf
                        ...
                        @foreach($vats as $vat)
                            <div class="flex items-center mb-2">
                                <input type="checkbox" id="vat_{{ $vat->id }}" name="vat_ids[]" value="{{ $vat->id }}"
                                       class="mr-2"
                                       {{ (is_array(old('vat_ids')) && in_array($vat->id, old('vat_ids'))) ? 'checked' : '' }} />
